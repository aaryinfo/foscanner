...
def get_today_data():
    try:
        ...
        # 1. Read Astro Score from pre-generated JSON (generated by Github Action)
        try:
            import json
            import os
            
            # Since Vercel includes *.json files if configured (we'll add it to vercel.json)
            # or just rely on it being in the repo
            json_path = os.path.join(os.path.dirname(__file__), 'astro_forecast.json')
            if os.path.exists(json_path):
                with open(json_path, 'r', encoding='utf-8') as f:
                    forecasts = json.load(f)
                    for f_data in forecasts:
                        if f_data['date'] == today_str:
                            score_data = f_data
                            break
                    if not score_data and len(forecasts) > 0:
                        score_data = forecasts[0] # Fallback to latest available
        except Exception as e:
            print(f"Error reading astro_forecast.json: {e}")
            
        # 2. If JSON is missing, fallback to live calculation (using MockSwe on Vercel)
        if not score_data:
        ...
        # 3. Read Stocks from JSON (generated by Github Action)
        stocks_data = []
        try:
            stocks_json_path = os.path.join(os.path.dirname(__file__), 'top_stocks.json')
